...
func getTwitter(cred *oauth.Credentials, urlStr string, params url.Values, data interface{}) error {
	...
	if resp.StatusCode != 200 {
		...
		return fmt.Errorf("Get %s returned status %d, %s", urlStr, resp.StatusCode, string(p))
	}
...
func serveLogin(w http.ResponseWriter, r *http.Request) {
	...
	tempCred, err := oauthClient.RequestTemporaryCredentials(http.DefaultClient, callback)
	if err != nil {
		http.Error(w, "Error getting temp cred, "+err.Error(), 500)
		return
	}
	addCookie(w, "temp", tempCred.Secret, 0)
	http.Redirect(w, r, oauthClient.AuthorizationURL(tempCred), 302)
}
